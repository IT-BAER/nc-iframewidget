...
- Nextcloud 34 support (`max-version` raised from 33 to 34; now supports Nextcloud 33 and 34)

### Fixed

- PHP 8.4/8.5 implicit-nullable parameter deprecation warnings in dashboard widget slot classes (`?\OCP\IUser` explicit-nullable typing)

### Removed

- Debug `error_log` statements from `GroupWidgetSlot::isEnabled()`
...
- **CSP handling**: Allow configured iframe domains via Nextcloud CSP API (public, group, and personal widgets) instead of relying on web-server CSP headers
- **Settings previews**: Apply CSP policy on admin/personal settings pages so previews load with the configured domains

### Changed

- **Documentation**: CSP guidance updated to avoid server-level header overrides
